...
flags.DEFINE_string("checkpoint_dir", "/disk2/zhaojiacheng/TP-GAN/models/", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("sample_dir", "/disk2/zhaojiacheng/TP-GAN/samples/", "Directory name to save the image samples [samples]")
#flags.DEFINE_boolean("is_train", True, "True for training, False for testing [False]")
flags.DEFINE_boolean("testing", False, "True for training, False for testing [False]")
...
class DCGAN(object):
    def __init__(self, sess, image_size=128, is_crop=True,
                 ...
                 checkpoint_dir=None, sample_dir=None, testing=False):
        """
        Args:
        ...
        if self.testing:
            #self.batch_size = 10
            self.testingphase = 'all'#'gt50'
            self.sample_run_num = 8762 // 2
        ...
        random.seed()
        self.DeepFacePath = '/disk2/zhaojiacheng/TP-GAN/DeepFace168.pickle'
    ...
    def build_model(self):
        ...
        if WITHOUT_CODEMAP:
            ...
            self.images = tf.split(self.images_with_code, 2, 3)[0]
            self.sample_images_nocode = tf.split(self.sample_images, 2, 3)[0]
        ...
        self.G_eyer,self.c_eyer = self.partRotator(tf.concat([self.eyer, self.eyel], axis=3), "PartRotator_eyer")
        ...
        self.G_eyer_sam, self.c_eyer_sam = self.partRotator(tf.concat([self.eyer_sam, self.eyel_sam],axis=3), "PartRotator_eyer", reuse=True)
        ...
        self.var_file = open('var_log.txt', mode='w')
    ...
    def train(self, config):
        ...
        data = MultiPIE(LOAD_60_LABEL=LOAD_60_LABEL, GENERATE_MASK=USE_MASK, RANDOM_VERIFY=RANDOM_VERIFY, MIRROR_TO_ONE_SIDE = True, source = self.testingphase, testing=self.testing)
        ...
        if not self.testing:
            ...
            self.f = open(self.logfile, mode='w')
            ...
            for epoch in xrange(config.epoch):
                ...
                for idx in xrange(0, batch_idxs):
                    ...
                    if np.mod(counter, self.sample_interval) == self.sample_interval-1:
                        ...
                        #testing accuracy
                        savedir = '/disk2/zhaojiacheng/TP-GAN/tem_test'
            ...
            for i in range(batchnum):
                ...
                if self.testimg:#Save images
                    ...
                    #                         '/disk2/zhaojiacheng/TP-GAN/{}/'.format(sample_dir),isOutput=False, filelist=filenames[ind[0]:ind[1]])
                    savedoutput += save_images(samples[5], [128, 128],
                                               '/disk2/zhaojiacheng/TP-GAN/{}/'.format(sample_dir),isOutput=True, filelist=filenames[ind[0]:ind[1]])
    ...
    def decoder(self, feat128, feat64, feat32, feat16, feat8, featvec,
        ...
        with tf.variable_scope(name) as scope:
            if reuse:
                scope.reuse_variables()
            initial_all = tf.concat([featvec, self.z], 1)
            ...
            before_select8 = resblock(tf.concat([initial_8, feat8], 3), k_h=2, k_w=2, name = "select8_res_1")
            #selection T module
            ...
            reconstruct16 = resblock(resblock(tf.concat([reconstruct16_deconv, before_select16], 3), name="dec16_res1"), name="dec16_res2")

            reconstruct32_deconv = relu(batch_norm(deconv2d(reconstruct16, [batch_size, 32, 32, self.gf_dim*4], name="g_deconv32"), name="g_bnd2"))
            before_select32 = resblock(tf.concat([feat32, g32_images_with_code, initial_32], 3), name = "select32_res_1")
            ...
            before_select64 = resblock(tf.concat([feat64, g64_images_with_code, initial_64], 3), k_h=5, k_w=5, name = "select64_res_1")
            ...
            before_select128 = resblock(tf.concat([feat128, initial_128, g128_images_with_code],3), k_h = 7, k_w = 7, name = "select128_res_1")
    ...
    def partRotator(self, images, name, batch_size=10, reuse=False):
        ...
        with tf.variable_scope(name) as scope:
            ...
            #up1
            after_select_d1 = lrelu(batch_norm(conv2d(tf.concat([d1, c2r], axis=3), self.gf_dim*4, d_h=1, d_w=1, name="p_deconv1_s"),name="p_bnd1_s"))
            ...
            #up2
            after_select_d2 = lrelu(batch_norm(conv2d(tf.concat([d2, c1r], axis=3), self.gf_dim*2, d_h=1, d_w=1, name="p_deconv2_s"),name="p_bnd2_s"))
            ...
            #up3
            after_select_d3 = lrelu(batch_norm(conv2d(tf.concat([d3, c0r], axis=3), self.gf_dim, d_h=1, d_w=1, name="p_deconv3_s"),name="p_bnd3_s"))
    ...
    def evaluate(self,epoch, idx, batch_idxs, start_time, mode,
        ...
        tobePrint = "%sE%2d %4d/%4d %2d d_l:%.4f" % (MODE + 'T' if mode == 'test' else '', epoch, idx, batch_idxs, int((time.time() - start_time)/3600), errD)
    ...
    def load(self, checkpoint_dir):
        ...
        if self.testing:
            model_dir = os.listdir(checkpoint_dir)[0]
        else:
            model_dir = "%s_%s_%s" % (self.dataset_name, self.batch_size, self.output_size)
