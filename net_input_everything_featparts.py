...
class MultiPIE():
    ...
    def __init__(self, datasplit='train', Random=True, LOAD_60_LABEL=False, MIRROR_TO_ONE_SIDE=True, RANDOM_VERIFY=False,
                 GENERATE_MASK=False, source='all', testing = False):
        self.dir = '/disk2/zhaojiacheng/TP-GAN/MultiPIE/cropped_img/'
        self.csvpath = '/disk2/zhaojiacheng/TP-GAN/MultiPIE/{}.csv'
        self.feat5ptDir = '/disk2/zhaojiacheng/TP-GAN/MultiPIE/trans_keypoint/'
        self.test_dir = '/disk2/zhaojiacheng/TP-GAN/MultiPIE/{}.csv'
        ...
        if not testing:
            split_f = self.csvpath.format(self.split)
            split_f_test = self.csvpath.format('test')
    ...
    def load_label_mask(self, filename, labelnum=-1):
        ...
        labelname = self.findSameIllumCodeLabelpath(filename)
    ...
    def GetFeatureParts(self, img_resize, filename, label=False):
        ...
        if True:#not label:
            featpath = self.feat5ptDir + filename[0:-7] + '.5pt'
            with open(featpath, 'r') as csvfile:
                reader = csv.reader(csvfile, delimiter=' ')
                for ind,row in enumerate(reader):
                    trans_points[ind,:] = row[0].split(',')
        ...
        if crop_x_end > img_resize.shape[1]:
            crop_x -= crop_x_end - img_resize.shape[1]
        elif crop_x < 0:
            crop_x_end += -crop_x
...
if __name__ == '__main__':
    dataset = MultiPIE()
    dataset.load_label_mask(dataset.indices[0])
